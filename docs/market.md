...
1. ~~**크레딧이 무엇을 사주는지 실측으로 말하기.**~~ **DAY 22 에
   만들었다.** 끝난 프로젝트의 중앙값·p90 을 세고, 세 건이 모이면
   요금제 화면이 저절로 "추정입니다"에서 "끝난 프로젝트 N건에서 잰
   값입니다"로 바뀐다. 아직 실제 모델로 돌린 적이 없어 지금은 표본이
   0 이다 — **키가 들어오는 순간 숫자가 채워진다.**
2. ~~**실행 기록을 되짚을 수 있게.**~~ **DAY 22 에 만들었다.** 판본마다
   누가·몇 라운드에·어떤 지적을 받고 고쳤는지가 남고, 산출물 화면에서
   파일 하나를 짚으면 그 경위가 펼쳐진다. 옛 판본에는 그 정보가 없어서
   빈 자리로 둔다 — 없는 것을 채우면 이력이 거짓말이 된다.
