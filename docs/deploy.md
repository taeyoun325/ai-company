...
보안 쪽(로그인 잠금)과 돈 쪽(동시 실행 좌석)은 DAY 22 에 없앴다. 좌석은
이제 색인에서 센다 — 실행 중에는 20초마다 박자가 메타를 쓰므로
`updated_at` 이 곧 "살아 있음"이고, 3분 넘게 멈춘 줄은 세지 않는다(죽은
인스턴스가 좌석을 영영 잠그지 않게).

남은 하나는 **순서**의 문제다. 같은 프로젝트에 두 인스턴스가 동시에
MANUAL 지시를 받으면 둘 다 통과한다. 파일 쓰기는 각자 원자적이지만
마지막에 쓴 쪽이 이긴다. 지금도 **인스턴스 하나로만 돌리는 것이
안전하다.**
