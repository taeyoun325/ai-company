...
쓰므로 `updated_at` 이 곧 "살아 있음"이고, 3분 넘게 멈춘 줄은 세지 않는다),
MANUAL 은 만료 시각이 있는 임대로 잡는다(잠근 인스턴스가 죽어도 풀린다).

**실시간 로그(SSE)도 DAY 23 에 옮겼다** — 정확히는, 이미 쓰고 있던
트레이스 파일(`{run}.jsonl`)을 진실로 삼는 쪽으로. 실행은 프로세스 하나
에서만 돌고 그 프로세스가 그 파일을 쓰므로, 프로젝트 하나를 보는
스트림(`run` 이 있는 `/api/stream`·`/api/events`)은 파일을 따라가면
어느 인스턴스에서 보든 같은 로그가 나온다(`bus.tail_trace`). 대가는
폴링 지연(기본 0.3초)뿐이다.

**남은 것은 전체 대시보드(`run` 없이 여러 프로젝트를 한 화면에 모아 보는
뷰) 하나다.** 이벤트 `id` 가 프로세스마다 따로 세는 일련번호라, 여러
