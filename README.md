...
| 요금제 5종 · 자체 키(BYOK) | ✅ 요금제가 **누구의 키로 부를지**를 정한다 |
...
**요금제가 누구의 키로 부를지를 정한다.** 무료는 Mock 이라 실제 모델에
닿지 않고, 자체 키(BYOK) 고객은 자기 키로만 돌고, 유료는 운영자 키로 돌며
그 비용만 크레딧에서 깎인다. 이 분기는 `app/tenant.py` **한 곳**에 있다.
...
- **인스턴스를 하나만 띄워야 한다.** 실행 상태가 프로세스 메모리에 있다.
